// Handles POST /api/apply — receives the MCAT coaching application,
// verifies the Cloudflare Turnstile captcha token, and emails the
// application to stemsquirrel@gmail.com via Resend. Both the Turnstile
// secret and the Resend API key stay server-side; the browser never sees
// them.
//
// SETUP REQUIRED (one time):
//   1. Cloudflare dashboard → Turnstile → Add a site. Choose "Managed"
//      widget type. Copy the SITE KEY into application.html
//      (search for the cf-turnstile div's data-sitekey) and the
